...
exports.applyCompletedCheckoutSession = void 0;
...
const getCheckoutRedirectUrl = (value, fallback) => {
    if (typeof value !== 'string' || !value.trim())
        return fallback;
    return (0, origins_1.isTrustedClientOrigin)(value.trim()) ? value.trim() : fallback;
};
const applyCompletedCheckoutSession = async (session) => {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
        try {
            ...
            if (error?.code === 'P2002') {
                return prisma_1.prisma.user.findFirst({ where: { id: userIdFromReference, deletedAt: null } });
            }
...
exports.applyCompletedCheckoutSession = applyCompletedCheckoutSession;
    ...
    static async getStripeState(req, res) {
        try {
            ...
            if (lastCheckoutSessionId && initialStripeState?.lastCheckoutStatus !== 'completed') {
                ...
                const updatedUser = await (0, exports.applyCompletedCheckoutSession)(checkoutSession);
    ...
    static async handleWebhook(req, res) {
        ...
        try {
            if (event.type === 'checkout.session.completed') {
                ...
                await (0, exports.applyCompletedCheckoutSession)(session);
            }
