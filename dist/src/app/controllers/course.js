...
const normalizeTee = (tee) => ({
    ...
    slopeWomen: tee?.slopeWomen === null || tee?.slopeWomen === undefined || tee?.slopeWomen === ''
        ? null
        : Number(tee.slopeWomen),
    slopeFrontWomen: tee?.slopeFrontWomen === null ||
        tee?.slopeFrontWomen === undefined ||
        tee?.slopeFrontWomen === ''
        ? null
        : Number(tee.slopeFrontWomen),
    slopeBackWomen: tee?.slopeBackWomen === null || tee?.slopeBackWomen === undefined || tee?.slopeBackWomen === ''
        ? null
    ...
    ratingWomen: tee?.ratingWomen === null || tee?.ratingWomen === undefined || tee?.ratingWomen === ''
        ? null
        : Number(tee.ratingWomen),
    ratingFrontWomen: tee?.ratingFrontWomen === null ||
        tee?.ratingFrontWomen === undefined ||
        tee?.ratingFrontWomen === ''
        ? null
        : Number(tee.ratingFrontWomen),
    ratingBackWomen: tee?.ratingBackWomen === null ||
...
class CourseController {
    ...
    static createCourse = async (req, res) => {
        try {
            ...
            const newCourse = await course_1.default.create(courseData);
    ...
    static updateCourse = async (req, res) => {
        try {
            ...
            const incomingTees = Array.isArray(course?.tees)
                ? course.tees.map((tee) => ({
            ...
            await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (deleteCandidates.length > 0) {
                    ...
                    const referencedIds = new Set([
                        ...evtRefs.map((e) => e.teeId),
                        ...roundRefs.map((r) => r.teeId),
                        ...scoreRefs.map((s) => s.teeId),
                    ]);
    ...
    static deleteCourse = async (req, res) => {
        try {
            const id = Number(req.params.id);
            const deletedCourse = await course_1.default.delete(id);
