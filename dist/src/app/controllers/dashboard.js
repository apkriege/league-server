...
class Dashboard {
    static async getPlayerEvents(req, res) {
        try {
            const playerId = Number(req.params.playerId);
            const leagueId = await prisma_1.prisma.player
                .findUnique({
                where: { id: playerId },
            ...
            const flightIds = await prisma_1.prisma.flight_player.findMany({
    ...
    static async getPlayerStats(req, res) {
        try {
            ...
            const averageNetScore = rounds.reduce((acc, round) => acc + round.net, 0) / (totalEvents || 1);
    ...
    static async getLeagueStats(req, res) {
        const leagueId = Number(req.params.leagueId);
        const playerId = Number(req.query.playerId);
        const eventIds = await prisma_1.prisma.event
            .findMany({
            where: { leagueId },
            select: { id: true },
        })
        ...
        const totalEvents = await prisma_1.prisma.event.count({
            where: { leagueId },
        });
        const averageScore = rounds.reduce((acc, round) => acc + round.gross, 0) / (rounds.length || 1);
        const averageNetScore = rounds.reduce((acc, round) => acc + round.net, 0) / (rounds.length || 1);
        let totals = rounds.reduce((acc, round) => {
            acc.eagles += round.eagles || 0;
            acc.birdies += round.birdies || 0;
            acc.pars += round.pars || 0;
            acc.bogeys += round.bogeys || 0;
    ...
    static async getLeagueLeaderboards(req, res) {
        try {
            const leagueId = Number(req.params.leagueId);
