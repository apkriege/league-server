...
class OperationsController {
    ...
    static createLeagueInvitations = async (req, res) => {
        const leagueId = Number(req.params.leagueId);
        ...
        const emails = Array.isArray(req.body?.emails)
            ? req.body.emails.map((email) => normalizeEmail(email)).filter(Boolean)
            : [];
        ...
        const inviteTargets = [
            ...
            ...emails.map((email) => ({ playerId: null, email, name: email })),
        ...
        for (const target of inviteTargets) {
            ...
            const invite = await prisma_1.prisma.league_invitation.create({
                data: {
                    leagueId,
    ...
    static revokeLeagueInvitation = async (req, res) => {
        const leagueId = Number(req.params.leagueId);
    ...
    static getInvitationByToken = async (req, res) => {
        ...
        if (invitation.expiresAt && invitation.expiresAt < new Date()) {
            await prisma_1.prisma.league_invitation.update({
    ...
    static claimInvitation = async (req, res) => {
        ...
        if (!player) {
            player = await prisma_1.prisma.player.findFirst({
        ...
        const claimed = await prisma_1.prisma.league_invitation.update({
            where: { id: invitation.id },
            data: {
                status: 'claimed',
                claimedById: userId,
    ...
    static getLeagueOnboarding = async (req, res) => {
        const leagueId = Number(req.params.leagueId);
        const [league, onboarding] = await Promise.all([
            prisma_1.prisma.league.findUnique({
                where: { id: leagueId },
                include: {
            ...
            prisma_1.prisma.league_onboarding.upsert({
                where: { leagueId },
                create: { leagueId },
                update: {},
            }),
        ...
        const steps = [
            ...
            },
            {
                ...
                complete: league.format !== 'team' || Boolean(onboarding.teamsReviewedAt) || league.teams.length > 0,
            },
            {
                key: 'firstEvent',
            ...
            },
            {
                ...
                complete: Boolean(onboarding.firstScoresEnteredAt) ||
    ...
    static updateLeagueOnboarding = async (req, res) => {
        const leagueId = Number(req.params.leagueId);
        const userId = Number(req.session.userId);
        ...
        if (!data)
            return res.status(400).json({ message: 'Invalid onboarding key' });
        const onboarding = await prisma_1.prisma.league_onboarding.upsert({
            where: { leagueId },
    ...
    static getLeagueAuditLogs = async (req, res) => {
        const leagueId = Number(req.params.leagueId);
        const logs = await prisma_1.prisma.audit_log.findMany({
            where: { leagueId },
    ...
    static createLeagueNotification = async (req, res) => {
        const leagueId = Number(req.params.leagueId);
        const userId = Number(req.session.userId);
        ...
        if (includeAdmin)
            recipientIds.add(league.adminId);
