...
exports.Scoring = exports.prisma = void 0;
...
class Scoring {
    ...
    constructor(event, scores) {
        ...
        this.tee = event.tee;
        this.scores = scores;
        this.players = event.flights.flatMap((flight) => flight.players.map((p) => p.player));
        this.teams = event.flights.flatMap((flight) => flight.teams.map((t) => t.team));
    }
    async run() {
        try {
    ...
    async modelPlayer(playerId, playerScores) {
        const player = this.players.find((p) => p.id === playerId);
        const scores = this.modelScores(player, playerScores);
        const stats = this.calculateStats(scores);
        const rating = this.event.holes === 9
            ? this.event.startSide === 'front'
                ? this.tee.ratingFrontMen
                : this.tee.ratingBackMen
            : this.tee.ratingMen;
        const slope = this.event.holes === 9
            ? this.event.startSide === 'front'
                ? this.tee.slopeFrontMen
                : this.tee.slopeBackMen
            : this.tee.slopeMen;
        // will change this to adjusted gross score later
        const { handicap, differential } = await this.calculateHandicapIndex(playerId, stats.totalAdjusted, this.tee.ratingMen, this.tee.slopeMen);
        const newScore = await exports.prisma.score.create({
            data: {
        ...
        await exports.prisma.scores.createMany({
            data: scores.map((s) => ({
                eventId: this.event.id,
                playerId: s.playerId,
                scoreId: newScore.id,
        ...
        await exports.prisma.player.update({
            where: { id: player.id },
            data: { handicap: Number(handicap.toFixed(2)) },
        });
    }
    async calculateHandicapIndex(playerId, score, rating, slope) {
        // last number of rounds to use
        const roundsToUse = 5;
        const player = await exports.prisma.player.findUnique({
            where: { id: playerId },
            include: {
                scores: {
    ...
    modelScores(player, scores) {
        ...
        const modeledScores = Object.entries(scores).map(([hole, score]) => {
            const h = this.tee.holes.find((ho) => ho.num === parseInt(hole));
            return {
                playerId: player.id,
                hole: parseInt(hole),
                par: h ? h.par : 4,
    ...
    calculateNetScores(playerHcp, scores) {
        const netScores = {};
        const popsMap = this.calculatePops(playerHcp, this.tee.holes, this.event.startSide, this.event.holes);
        for (const [hole, score] of Object.entries(scores)) {
            const holeNumber = Number(hole);
            const strokesOnHole = popsMap.get(holeNumber) || 0;
            netScores[holeNumber] = Math.max(0, score - strokesOnHole);
        }
        return netScores;
    }
    calculateAdjustedScores(playerHcp, scores) {
        const adjustedHoles = {};
        for (const [hole, score] of Object.entries(scores)) {
    ...
    calculatePops(hcp, holes, side, numHoles) {
        const startingHole = side === 'front' ? 1 : 10;
        const holesToUse = holes.filter((hole) => hole.num >= startingHole && hole.num < startingHole + numHoles);
    ...
    calculateStats(scores) {
        ...
        for (const score of scores) {
            stats.totalGross += score.score;
            stats.totalNet += score.net;
            stats.totalAdjusted += score.adjusted;
            // Gross stats
            const grossDiff = score.score - score.par;
            if (grossDiff <= -2)
                stats.eagles++;
            else if (grossDiff === -1)
                stats.birdies++;
...
const calculateHoleWinner = async (p1ScoreId, p2ScoreId, p1Net, p2Net, hole, ptsPerHole) => {
    let p1Points = 0;
    let p2Points = 0;
    if (p1Net < p2Net) {
        p1Points = ptsPerHole;
...
const calculatePoints = async (eventId) => {
    ...
    for (const flight of flights) {
        ...
        const t1Players = team1.players.sort((a, b) => a.scores[0].preHandicap - b.scores[0].preHandicap);
        const t1A = t1Players[0];
        const t1B = t1Players[1];
        const t2Players = team2.players.sort((a, b) => a.scores[0].preHandicap - b.scores[0].preHandicap);
        ...
        for (const score of t1A.scores[0].scores) {
            ...
            const t2ANet = t2A.scores[0].scores.find((s) => s.hole === score.hole)?.net || 0;
            const [p1Points, p2Points] = await calculateHoleWinner(t1A.scores[0].id, t2A.scores[0].id, t1ANet, t2ANet, score.hole, event.ptsPerHole);
            t1Ascore += p1Points;
            t2Ascore += p2Points;
        }
        ...
        for (const score of t1B.scores[0].scores) {
            ...
            const t2BNet = t2B.scores[0].scores.find((s) => s.hole === score.hole)?.net || 0;
            const [p1Points, p2Points] = await calculateHoleWinner(t1B.scores[0].id, t2B.scores[0].id, t1BNet, t2BNet, score.hole, event.ptsPerHole);
            t1Bscore += p1Points;
            t2Bscore += p2Points;
        }
...
const updatePlayerOpponent = async (scoreId, opponentId) => {
    await exports.prisma.score.update({
        where: { id: scoreId },
        data: {
            opponentId,
        },
...
const updatePlayerPoints = async (scoreId, points) => {
    await exports.prisma.score.update({
        where: { id: scoreId },
        data: {
            pointsEarned: points,
...
const updateHolePoints = async (scoreId, hole, points) => {
    await exports.prisma.scores.update({
        where: {
            scoreId_hole: {
                scoreId,
                hole,
...
const updateTeamPoints = async (teamId, points) => {
    await exports.prisma.team.update({
        where: { id: teamId },
        data: {
            seasonPoints: {
                increment: points,
